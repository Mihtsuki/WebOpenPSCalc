...
// Site-level FAQ. The home page is NOT generated by this script (index.html is
// hand-maintained and owns the SPA mount point), so these answers live in two
// places by necessity: here, for llms.txt, and there, as visible text plus
// FAQPage JSON-LD. `assertHomeFaqInSync` below fails the build if they drift —
// structured data that no longer matches the page is a markup violation, and
// silently divergent answers are worse than none.
const SITE_FAQ = [
  { q: "Is Open PS Calc official?",
    a: "No. It is an unofficial, free, fan-made community project, not affiliated with or endorsed by Payon Stories." },
  { q: "Which classes does the Payon Stories damage calculator support?",
    a: "All 16 base and second-job classes, including Gunslinger, Ninja and Super Novice. Transcendent and third-job classes are out of scope, because Payon Stories is a pre-renewal server." },
  { q: "Does it model Payon Stories' reworked skills, or vanilla Ragnarok Online?",
    a: "The server's own reworks. Formulas follow the Payon Stories wiki and the class-rework notes — Asura Strike, Grand Cross, Reflect Shield, the Hunter traps, Acid Terror and the rest all use the server's values, not vanilla pre-renewal ones. Where the server matches vanilla, the vanilla formula is used and marked as such." },
  { q: "What does the calculator do when it cannot model something?",
    a: "It says so instead of showing a number. A skill whose Payon Stories formula is not documented is shown with its element and type but no damage figure, and a vanilla fallback is flagged in the breakdown, so a fabricated number is never presented as a real one." },
  { q: "Can I share a build with someone else?",
    a: "Yes. The whole build — stats, gear, cards, skill, buffs and target — is encoded in the page URL, so copying the address bar shares the exact build. No account or sign-in is needed." },
  { q: "Does it show how the damage number was reached?",
    a: "Yes. Every result is a step-by-step breakdown — base damage, skill ratio, defence, masteries, element and cards, in the order the server applies them — alongside ASPD, cast and hit breakpoints, time-to-kill against any monster, and incoming damage for survivability." },
];

// The two copies on the home page are checked SEPARATELY and against different
// sources. Checking a tag-stripped whole file would be worthless: JSON-LD lives
// inside <script>, so its own text would satisfy a "is it visible?" test and the
// guard would pass while the visible FAQ said something else entirely.
function assertHomeFaqInSync() {
  const home = fs.readFileSync(path.join(__dirname, "..", "index.html"), "utf8");
  const errs = [];
  const norm = (s) => s.replace(/\s+/g, " ").trim();

  // 1. The FAQPage JSON-LD must match SITE_FAQ exactly, question for question.
  const ldBlocks = [...home.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)]
    .map((m) => { try { return JSON.parse(m[1]); } catch { return null; } });
  const ld = ldBlocks.find((b) => b && b["@type"] === "FAQPage");
  if (!ld) {
    errs.push("index.html has no parseable FAQPage JSON-LD block");
  } else {
    const got = ld.mainEntity.map((e) => ({ q: norm(e.name), a: norm(e.acceptedAnswer.text) }));
    if (got.length !== SITE_FAQ.length) errs.push(`FAQPage has ${got.length} entries, SITE_FAQ has ${SITE_FAQ.length}`);
    SITE_FAQ.forEach(({ q, a }, i) => {
      if (!got[i]) return;
      if (got[i].q !== norm(q)) errs.push(`FAQPage Q${i + 1} question differs:\n    schema: ${got[i].q}\n    source: ${norm(q)}`);
      else if (got[i].a !== norm(a)) errs.push(`FAQPage answer differs for "${norm(q)}"`);
    });
  }

  // 2. The VISIBLE block — markup unbacked by on-page content is a violation.
  const vis = home.match(/<div id="seo-faq">([\s\S]*?)<\/div>\s*<\/div>/);
  if (!vis) {
    errs.push('index.html has no <div id="seo-faq"> block for the FAQ to be visible in');
  } else {
    const text = norm(vis[1].replace(/<[^>]+>/g, " "));
    for (const { q, a } of SITE_FAQ) {
      if (!text.includes(norm(q))) errs.push(`visible FAQ is missing the question: ${norm(q)}`);
      else if (!text.includes(norm(a))) errs.push(`visible FAQ answer differs for: ${norm(q)}`);
    }
  }

  if (errs.length) {
    throw new Error(
      "index.html's FAQ has drifted from SITE_FAQ in gen-guides.mjs.\n"
      + "Update all three: SITE_FAQ, the visible #seo-faq block, and the FAQPage JSON-LD.\n  "
      + errs.join("\n  ")
    );
  }
}
...
function shell({ title, desc, canonical, body, jsonLd = [], robots = null, modified = null }) {
...
<meta property="og:site_name" content="Open PS Calc">
<meta property="og:image" content="${SITE}/icon-512.png">
<meta property="og:image:type" content="image/png"><meta property="og:image:width" content="512"><meta property="og:image:height" content="512"><meta property="og:image:alt" content="Open PS Calc logo">${modified ? `
<meta property="article:modified_time" content="${modified}">` : ""}
<meta name="twitter:card" content="summary"><meta name="twitter:title" content="${esc(title)}">
<meta name="twitter:description" content="${esc(desc)}"><meta name="twitter:image" content="${SITE}/icon-512.png">
<meta name="twitter:image:alt" content="Open PS Calc logo">
...
<footer>Open PS Calc is an unofficial, fan-made <a href="/">Payon Stories damage calculator</a>. Stats are a starting point — tune them to your gear and goals.</footer>`;
  return shell({ title, desc, canonical, body, jsonLd, modified: LASTMOD.guides });
}
...
<footer>Open PS Calc is an unofficial, fan-made <a href="/">Payon Stories damage calculator</a>. Not affiliated with or endorsed by Payon Stories.</footer>`;
  return shell({ title, desc: m.blurb, canonical, body, jsonLd, modified: LASTMOD.mechanics });
}
...
<footer>Open PS Calc is an unofficial, fan-made <a href="/">Payon Stories damage calculator</a>. Where a formula isn't published, the calculator says so rather than showing an invented number.</footer>`;
  return shell({
    ...
    canonical, body, jsonLd, modified: LASTMOD.mechanics,
  });
}
...
<footer>Open PS Calc is an unofficial, fan-made <a href="/">Payon Stories damage calculator</a>.</footer>`;
  return shell({ title: "Payon Stories Build Guides — every class | Open PS Calc", desc: "Starter build guides for every Payon Stories class (pre-renewal RO): recommended stats and signature skills, ready to open in the damage calculator.", canonical, body, jsonLd, modified: LASTMOD.guides });
}
...
Answers are kept in step with the FAQ shown on the home page and its FAQPage structured data.
${SITE_FAQ.map(({ q, a }) => `- **${q}** ${a}`).join("\n")}
